using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;
using System.ComponentModel.DataAnnotations;

namespace FutureValue.Models
{
    public class FutureValueModel
    {
        private decimal? _monthlyInvestment;
        private decimal? _yearlyInterestRate;
        private int? _years;


        [Required(ErrorMessage ="Please enter a monthly investment amount.")]
        [Range(1,500, ErrorMessage ="Monthly investment amount must be between 1 and 500.")]
        public decimal? MonthlyInvestment
        { 
            get { return _monthlyInvestment; }
            set { _monthlyInvestment = value; }
        }
        [Required(ErrorMessage = "Please enter a yearly interest amount.")]
        [Range(0.1, 10.0, ErrorMessage = "Yearly interest rate must be between 0.1 and 10.0.")]
        public decimal? YearlyInterestRate
        {
            get { return _yearlyInterestRate; }
            set { _yearlyInterestRate = value; }
        }
        [Required(ErrorMessage = "Please enter a number of years.")]
        [Range(1, 500, ErrorMessage = "Number of years must be between 1 and 50.")]
        public int? Years
        {
            get { return _years; }
            set { _years = value; }
        }


        public decimal? CalculateFutureValue()
        {
            int? months = _years * 12;
            decimal? monthlyInterestRate = _yearlyInterestRate / 12 / 100;
            decimal? futureValue = 0;
            for (int i = 0; i < months; i++)
            {
                futureValue = (futureValue + _monthlyInvestment) * (1 + monthlyInterestRate);
            }
            return futureValue;
        }
    }
}
